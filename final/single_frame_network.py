...
# Network Parameters
n_input_x = 60 # Input image x-dimension
...
def input_data():
    path = "/home/kendall/Documents/Development/CS463G/final/data/60x80/single_frame/"
...
def cnn(x, weights, biases):
    ...
    frame = tf.reshape(x, [-1, 60, 80, 3, 1])

    # BEGIN CNN
    with tf.name_scope("conv1") as scope:
        conv1 = conv3d(frame, weights['wc1'], biases['bc1'], strides=2)
...
weights = {
    'wc1' : tf.get_variable("weights_1", shape=[3, 3, 3, 1, 4],
...
with tf.Session() as sess:
    ...
    np.savetxt("./overall_loss_single_frame.csv", overall_loss, delimiter=",")
    np.savetxt("./overall_acc_single_frame.csv", overall_acc, delimiter=",")

    prediction_acc = np.array(prediction_acc)
    np.savetxt("./pred_acc_single_frame.csv", prediction_acc, delimiter=",")
    ...
    np.savetxt("./mean_acc_by_class_single_frame.csv", np.mean(prediction_acc_by_class, axis=0) / prediction_class_distribution, delimiter=",")
    np.savetxt("./acc_by_class_single_frame.csv", prediction_acc_by_class, delimiter=",")
