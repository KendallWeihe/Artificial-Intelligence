...
Files:
  - main.py: this will run the search on the 5 pre-randomized puzzles I have generated
  - ida_search.py: this is the program that actually implements the IDA* algorithm
  - antons_randomizer/* : the files for Anton's randomizer
  - mlp_heuristic.py: this is the file that trains the MLP
  - ida_mlp.py: this is the file that uses the trained MLP with the IDA* algorithm
  - heuristic.ckpt: the save MLP model
  - test/* : directory of test puzzles -- main.py makes use of these
  - ground_truth.tar.gz: ground truth for the MLP
...
Citations:
  ...
  - I used Tensorflow code to build a neural network from here: https://github.com/aymericdamien/TensorFlow-Examples/
...
Heuristic:
  ...
  I built a Multilayered Perceptron (MLP) neural network in an attempt to
    ...
    results consistent with Anton's heuristic. I have included the code for the
    MLP -- both training, and testing -- but it may take more time than
    is worth it for you. You will need to install Tensorflow.
...
If you want to run the MLP:
  Install Tensorflow: https://www.tensorflow.org/versions/r0.10/get_started/os_setup.html

  1. Training:
    - unzip the ground truth tar file
    - execute `python mlp_heuristic.py`
  3. Using the MLP for computing a heuristic:
    - execute `python ida_mlp.py`



What I learned from this assignment:
  - I learned hands on experience of implementing an IDA* search algorithm
  - I learned how to use python scripts to start subprocesses
  - It was reinforced that neural networks are typically only as good as their ground truth
  - I learned various new NumPy functions:
    - numpy.flipud()
    - numpy.intersect1d()
    - numpy.vstack()
  - I learned how to export data from a JS function to a CSV file
  - I learned implementing IDA* in code is an involved process
